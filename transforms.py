...
class RandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('bbox_fields', []):
            ...
            # 检查中心点是否在 crop 区域内
            valid_inds = (center_x >= crop_x1) & (center_x < crop_x2) & \
                        (center_y >= crop_y1) & (center_y < crop_y2)
            # ========== 修改结束 ==========
            
            # 如果该 crop 不包含任何 gt-bbox 的中心点且 allow_negative_crop 为 False，跳过该图像
            if (key == 'gt_bboxes' and not valid_inds.any()
                    and not allow_negative_crop):
                return None
            
            # 对保留的 bbox 进行坐标变换（减去 offset）
            bbox_offset = np.array([offset_w, offset_h, offset_w, offset_h],
                                dtype=np.float32)
            bboxes = bboxes[valid_inds, :] - bbox_offset
